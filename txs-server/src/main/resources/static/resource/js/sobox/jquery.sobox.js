...
//			obj.css({'width':0,'height':0,'marginLeft':0,'marginTop':0}).show().animate({'width':w,'height':h,'marginLeft':-w/2,'marginTop':-h/2},'fast');
            obj.find('.s-close').bind('click', function () {
                that.hide(obj);
            });
            if (fn) {
                fn();
        ...
        pop: function (o) {
            ...
            var o = $.extend({
                ...
                ajax: {
                    url: null, data: null, callback: function () {
                    }
                },//ajax事件@
                ...
                /* 自定义参数 */
                cls: null,//添加自定义类名@
                width: 600, height: null,//宽高属性,iframe模式下，height为iframe高度@
                defaultShow: true,//直接显示pop@
                visibility: true,//默认pop执行后显示（用于部分复杂处理场景）@
                title: '系统提示',//默认标题@
                showTitle: true,//标题栏隐藏：默认显示@
            ...
            if (o.defaultShow && !onlyOne) {
                showPop();
            }//默认直接显示，,否则通过手动事件执行showPop，初始化显示pop
            ...
            function showPop(opt) {//showPop
                ...
                if (o.height && (o.type != 'iframe')) {
                    $wrap.css('height', o.height + 'px');
                }//添加自定义高
